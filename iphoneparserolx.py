...
# Список твоих поисков (iPhone 13 Pro, 14 Pro, 15 Pro)
OLX_URLS = [
    ...
    for url in OLX_URLS:
        try:
            ...
            if response.status_code != 200:
                print(f"OLX ответил статусом: {response.status_code}")
                continue
            ...
            for card in cards:
                ...
                # Отправляем в ТГ только новые объявления (после первого круга проверки)
                if len(seen_ads) > 0:
            ...
            time.sleep(3)  # Небольшая пауза между ссылками, чтобы OLX не банил
...
if __name__ == "__main__":
    send_telegram("🚀 Парсер запущен на Render и отслеживает 13 Pro, 14 Pro и 15 Pro!")
    while True:
        check_olx()
        time.sleep(90)  # Полный круг каждые 1.5 минуты
